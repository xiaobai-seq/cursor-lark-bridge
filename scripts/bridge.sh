...
start_daemon() {
    if is_daemon_running; then
        echo -e "${YELLOW}daemon 已在运行 (PID=$(read_pid_from_file "$PID_FILE"))${NC}"
        return 0
    fi
    ...
    sleep 1
    if is_daemon_running; then
        echo -e "${GREEN}daemon 已启动 (PID=$(read_pid_from_file "$PID_FILE"))${NC}"
    else
...
kill_daemon() {
    if is_daemon_running; then
        ...
        if [ -n "$pid" ]; then
            ...
            # PID 文件格式损坏（JSON 解析失败且首行不是数字）——
            # 不冒险调 `kill ""`，直接留给下面的 rm -f 收尾
            echo -e "${YELLOW}PID 文件格式异常，跳过信号发送${NC}"
        fi
        rm -f "$PID_FILE"
    ...
    if [ -f "$PID_FILE" ]; then
        ...
        if [ -n "$pid_in_file" ] && ! kill -0 "$pid_in_file" 2>/dev/null; then
            ...
            if [ -n "$actual_daemon" ] && [ "$actual_daemon" != "$pid_in_file" ]; then
                ...
                # v0.2 起 PID 文件是 JSON（含 start_ts/version/reconnect_count），
                # 不能再用 `echo $pid > file` 覆盖成单行裸 PID，会丢字段。
                # 让 daemon 自己启动时写一份完整 JSON 最稳妥。
                fixes+=("fb restart  # 让 daemon 自己重写 JSON PID 文件")
            else
...
cmd_service_install() {
    ...
    sleep 2
    if is_daemon_running; then
        echo -e "  ${GREEN}✓${NC} daemon 已通过 launchd 运行 (PID=$(read_pid_from_file "$PID_FILE"))"
    else
